...
static void readFromCgi(SocketFD fd, std::string& readBuffer, IN_OUT_STATE& state) {
...
void http::ClientEventProcessor::processClientEvents(int index) {
	...
	if (_allSockets[index].revents & POLLIN) {
		if (cgi) {
			...
			std::cout << "here fd=" << cgi->getOutputPipeFd() << " i=" << i << "\n";
			++i;
			readFromCgi(fd, cgi->getReadBuffer(), cgi->getState());
			if (cgi->getState() == CGI_COMPLETED)
				std::cout << "here fd=" << cgi->getOutputPipeFd() << "\n";
	...
	if (cgi && cgi->hasFinished() && cgi->getState() == CGI_COMPLETED) {
