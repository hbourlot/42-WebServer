...
void http::ClientEventProcessor::processRead(pollfd &pfd, Client *client, Cgi *cgi) {
	...
	SocketFD fd = pfd.fd;
	std::string &readBuffer = (cgi) ? cgi->getOutputBuffer() : client->getReadBuffer();
	if (cgi) {
		CLIENT_STATE dummyState = RESET;
		if (!readFromSocket(fd, readBuffer, dummyState))
			return;
	} else {
...
bool http::ClientEventProcessor::readFromSocket(SocketFD fd, std::string &readBuffer, CLIENT_STATE &state) {
