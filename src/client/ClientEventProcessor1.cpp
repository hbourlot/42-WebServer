...
static void readFromCgi(SocketFD fd, std::string& readBuffer, IN_OUT_STATE& state, http::Cgi* cgi) {
	...
	while (true) {
		...
		// std::cout << "on loop fd->" << fd << " res->" << bytesReceived << std::endl;
		if (bytesReceived > 0) {
			readBuffer.append(buffer, bytesReceived);
			cgi->bytesRead += bytesReceived;
			dataWasReadInThisCall = true;
			continue;

		} else if (bytesReceived == 0) {
			// Writer closed; treat as EOF and mark CGI completed.
			state = CGI_COMPLETED;
			break;
...
void http::ClientEventProcessor::processClientEvents(int index) {
	...
	if (cgi && cgi->hasFinished() /* && (_allSockets[index].revents & POLLHUP) */) {
		// readFromCgi(fd, cgi->getReadBuffer(), cgi->getState(), cgi);
		// if (cgi->getState() == CGI_COMPLETED) {
			client->setState(CGI_COMPLETED);
			// std::cout << "cgi->state=" << cgi->getState() << std::endl;
			// std::cout << "fd=>" << fd << std::endl;
			// std::cout << "all read=>" << cgi->bytesRead << "\n";
		// }
		return;
	}
}
