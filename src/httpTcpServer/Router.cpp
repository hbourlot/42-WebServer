...
MatchResult getMatchResult(const std::string &path, ServerConfig &server)
{
	MatchResult result;
	result.file = NULL;
...
static bool validateRequestMethod(const http::Request &request, const Location &location)
{
	...
	// std::cout << "Location not valid" + location.path + " " + request.method << std::endl;
	return false;
}
...
VALIDATION_STATUS http::Router::validateRequest(Client &client, const ServerConfig &server)
{
	...
	request.matchResult = getMatchResult(request.path, client.getServer().getServerInfo());
	...
	if (request.matchResult.file && request.matchResult.location)
	{
		// std::cout << "Vamos buscar os metodos" << std::endl;
		request.matchResult.file->methods = getAllMethods(server, request.path);
		// for (std::vector<std::string>::iterator it = request.matchResult.file->methods.begin();
		//      it != request.matchResult.file->methods.end(); ++it)
		// 	std::cout << *it << std::endl;
		// std::cout << "Conseguimos buscar os metodos" << std::endl;
	}
