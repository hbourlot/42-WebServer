...
void http::Response::initFromRequest(const http::Request &request) {
	...
	std::map<std::string, std::string>::const_iterator itRange = request.getHeaders().find("Range");

	if (itRange != request.getHeaders().end())
...
std::string http::Response::buildResponseString(void) {
