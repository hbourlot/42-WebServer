...
void http::TcpServer::processClientEvents() {
	...
	for ( size_t idx = 1; idx < _fds.size(); ++idx ) {
		...
		if ( _fds[ idx ].revents & POLLIN ) {
			...
			if ( requestStatus == MAIN_FUNCTIONALITY_PROBLEMS ) {
				// * Should close the specific client
			} else if ( requestStatus == REQUEST_EMPTY ) {
				...
				// * Parse Request
				requestStatus = parseRequest( _clientManager.getClient( fd ), _serverInfo );

				if ( requestStatus == PARSE_INCOMPLETE ) {
					// * Move further
					continue;
				} else if ( requestStatus == PARSE_TOO_LARGE ) {
