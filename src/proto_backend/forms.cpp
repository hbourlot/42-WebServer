...
protoBackend set_s_protoBackend(std::string statusCode, std::string statusMsg, std::string htmlFilePath)
{
    protoBackend result;
...
static protoBackend formurldeconded(httpRequest request)
{
  ...
  if (form["username"] == "admin" && form["password"] == "1234\n") {
    ...
    result = set_s_protoBackend("200", "OK", htmlFilePath);
    } else {
        std::string errorFilePath("./content/error_401.html");
    result = set_s_protoBackend("401", "Unauthorized", errorFilePath);
