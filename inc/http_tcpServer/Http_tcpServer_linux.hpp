...
namespace http {
	...
	class TcpServer {
		...
		private:
			...
			int sendResponse(SocketFD fd, std::string contentToSend);

			void prepareResponse(const HttpStatusCode &status, const std::string &body,
			...
			bool parseCgi(const Location loc, sockaddr_in &clientAddress, httpRequest &request, pollfd &clientSocket,
